...
function fillRest(m : number, k : number, words : number,
    ...
    if (m == 0) return;
    const last_x = !(avec[k] & (1 << (m-1))) &&  (bvec[k] & (1 << (m-1)));
    const last_z =  (avec[k] & (1 << (m-1))) && !(bvec[k] & (1 << (m-1)));
    if (last_x) {
        if (bitnum(m)) bvec[k] |= (-1) << m;
        if (k + 1 < words) bvec.fill(-1, k + 1);
    } else if (last_z) {
        if (bitnum(m)) avec[k] |= (-1) << m;
        if (k + 1 < words) avec.fill(-1, k + 1);
    }
}

// Each digit contributes independently: a normal digit i packs i into both
// the a-field (upper 16 bits of val) and b-field (lower 16 bits) -- (1,1)-style
// bits that get split apart bit-by-bit later. 'x' packs all-ones into the
// b-field only ((0,1) per bit). 'z' packs all-ones into the a-field only
// ((1,0) per bit).
...
function toHexInternal(start : number, bits : number, avec : Uint32Array, bvec : Uint32Array) {
    ...
    while (bit < bits) {
        ...
        const und = avec[k] ^ bvec[k];
        const zpart = avec[k] & ~bvec[k];
        k++;
        for (let b = 0; b < 8 && bit < bits; b++, bit += 4) {
            const m = 0xf << 4 * b;
            if ((und & m) == 0) out.push(a[a.length - 1 - b]);
            else if ((zpart & m) == m) out.push('z');
            else out.push('x');
        }
    }
...
function toBinInternal(start : number, bits : number, avec : Uint32Array, bvec : Uint32Array) {
    ...
    while (bit < bits) {
        ...
        for (let b = 0; b < 32 && bit < bits; b++, bit++) {
            if (!(und & (1 << b))) out.push(a[a.length - 1 - b]);
            else if (zpart & (1 << b)) out.push('z');
            else out.push('x');
        }
    }
    return out.reverse().join('');
 ...
 * * 2, 'z' mean high-impedance value.
 */
type InitType = 1 | 0 | -1 | 2 | boolean | '1' | '0' | 'x' | 'z';

/**
 * Type for clonable representations of four-value vectors.
 */
type Clonable4vl = {
    _bits: number;
    _avec: Uint32Array;
...
/**
 * Exception for four-value vectors.
 */
export class Error4vl extends Error {
    constructor(s: string) {
        super(s);
        Object.setPrototypeOf(this, Error4vl.prototype);
    }
}

function assert(c : boolean, s : string) {
    if (!c) throw new Error4vl("Assertion failed: " + s);
}

/**
 * Four-value logic vectors.
 *
 ...
 * * A: 1, B: 1 -- logical 1,
 * * A: 1, B: 0 -- high-impedance value, "z".
 *
 * Per IEEE 1800-2023 (SystemVerilog), the "z" value behaves identically to
 * "x" for every bitwise/reduction logic operator (Tables 11-11 through
 * 11-18): it never appears as the *result* of and/or/xor/nand/nor/xnor/not
 * or their reductions, only as an input. Operations below normalize "z"
 * bits on their operand(s) to "x" bits before reusing the same bit-twiddling
 * formulas that compute 0/x/1 results.
 */
export class Vector4vl {
    ...
    /**
     * Private constructor for four-value logic vectors.
     *
    ...
    static make(bits : number, init : InitType) {
        ...
        switch(init) {
            ...
            case 'z': case 2: iva = ~0; ivb = 0; break;
            default: assert(false, "Vector4vl.make() called with invalid initializer");
        }
        const words = (bits+31)/32 | 0;
        return new Vector4vl(bits,
    ...
    static xes(bits : number) {
        return Vector4vl.make(bits, 0);
    }

    /**
     * Construct a vector containing only high-impedance values.
     *
     * @param bits Number of bits in the vector.
     */
    static zes(bits : number) {
        return Vector4vl.make(bits, 2);
    }

    /**
     * An empty vector.
     */
    static empty = Vector4vl.zeros(0);

    /**
     * A single one.
     */
    static one = Vector4vl.ones(1);

    /**
     * A single zero.
     */
    static zero = Vector4vl.zeros(1);

    /**
     * A single undefined value.
     */
    static x = Vector4vl.xes(1);

    /**
     * A single high-impedance value.
     */
    static z = Vector4vl.zes(1);
    ...
    static fromBool(b : boolean, bits = 1) {
        return Vector4vl.make(bits, b ? 1 : -1);
    }
    ...
    static concat(...vs : Vector4vl[]) {
        ...
        return new Vector4vl(bits, avec, bvec);
    }
    ...
    static fromIterator(iter : Iterable<number>, skip : number, nbits : number) {
        if ((skip & (skip - 1)) == 0)
            return Vector4vl.fromIteratorPow2(iter, skip, nbits);
        else return Vector4vl.fromIteratorAnySkip(iter, skip, nbits);
    }
    ...
    static fromIteratorAnySkip(iter : Iterable<number>, skip : number, nbits : number) {
        ...
        if (m < nbits) fillRest(m, k, words, avec, bvec);
        return new Vector4vl(nbits, avec, bvec);
    }
     ...
     *
     * Four characters are accepted:
     * * '0' for logical 0,
     * * 'x' for undefined value,
     * * '1' for logical 1,
     * * 'z' for high-impedance value.
     *
     ...
     *
     * Characters '0' to '7', 'x' and 'z' are accepted. The characters 'x'
     * and 'z' mean three undefined/high-impedance bits, respectively.
     *
     * If _nbits_ is given, _data_ is either truncated, or extended with
     * undefined/high-impedance values.
     *
     ...
     *
     * Characters '0' to '9', 'a' to 'f', 'x' and 'z' are accepted. The
     * characters 'x' and 'z' mean four undefined/high-impedance bits,
     * respectively.
     *
     * If _nbits_ is given, _data_ is either truncated, or extended with
     * undefined/high-impedance values.
    ...
    static fromString(data : string) {
        const re = /^([0-9]*)'?(b[01xz]*|o[0-7xz]*|h[0-9a-fxz]*|d[0-9]*)$/;
        const res = re.exec(data);
        assert(res != null, "Vector4vl.fromString() Invalid string");
        const bits = res[1].length ? Number(res[1]) : undefined;
        const num = res[2].slice(1);
        switch (res[2][0]) {
            case 'b': return Vector4vl.fromBin(num, bits);
            case 'o': return Vector4vl.fromOct(num, bits);
            case 'h': return Vector4vl.fromHex(num, bits);
            case 'd': return Vector4vl.fromNumber(BigInt(num), bits);
        }
    }
     ...
     */
    static fromClonable(data : Clonable4vl) {
        return new Vector4vl(data._bits, data._avec, data._bvec);
    }
    ...
    /**
     * Most significant bit in the vector. Returns -1, 0, 1 or 2.
     */
    get msb() : number {
    ...
    /**
     * Least significant bit in the vector. Returns -1, 0, 1 or 2.
     */
    get lsb() : number {
        return this.get(0);
    }
    ...
    /**
     * Gets _n_th value in the vector. Returns -1 (0), 0 (x), 1 (1) or 2 (z).
     */
    get(n : number) {
        const bn = bitnum(n);
        const wn = wordnum(n);
    ...
    /**
     * Tests if every bit in the vector is defined (no x, no z).
     */
    ...
    /**
     * Turns high-impedance ("z") bits into undefined ("x") bits, leaving
     * 0/1/x bits untouched. Per IEEE 1800-2023 Tables 11-11 through 11-18,
     * "z" behaves identically to "x" for every bitwise/reduction operator,
     * so every such operator normalizes its operand(s) with this helper
     * before reusing the plain 0/x/1 bit-twiddling formulas.
     *
     * For internal use.
     */
    private _z2x() : [Uint32Array, Uint32Array] {
        return [
            zip((a, b) => a & b, this._avec, this._bvec),
            zip((a, b) => a | b, this._avec, this._bvec)
        ];
    }
     ...
     */
    and(v : Vector4vl) {
        assert(v._bits == this._bits, "Vector4vl.and() called with vectors of different sizes");
        const [a1, b1] = this._z2x();
        const [a2, b2] = v._z2x();
        return new Vector4vl(this._bits,
            zip((a, b) => a & b, a2, a1),
            zip((a, b) => a & b, b2, b1));
    }
     ...
     */
    or(v : Vector4vl) {
        assert(v._bits == this._bits, "Vector4vl.or() called with vectors of different sizes");
        const [a1, b1] = this._z2x();
        const [a2, b2] = v._z2x();
        return new Vector4vl(this._bits,
            zip((a, b) => a | b, a2, a1),
     ...
     */
    xor(v : Vector4vl) {
        assert(v._bits == this._bits, "Vector4vl.xor() called with vectors of different sizes");
        const [a1, b1] = this._z2x();
        const [a2, b2] = v._z2x();
        return new Vector4vl(this._bits,
            zip4((a1_, a2_, b1_, b2_) => (a1_ | b1_) & (a2_ ^ b2_), a2, b2, a1, b1),
            zip4((a1_, a2_, b1_, b2_) => (a1_ & b1_) ^ (a2_ | b2_), a2, b2, a1, b1));
    }
     ...
     */
    nand(v : Vector4vl) {
        assert(v._bits == this._bits, "Vector4vl.nand() called with vectors of different sizes");
        const [a1, b1] = this._z2x();
        const [a2, b2] = v._z2x();
        return new Vector4vl(this._bits,
            zip((a, b) => ~(a & b), b2, b1),
     ...
     */
    nor(v : Vector4vl) {
        assert(v._bits == this._bits, "Vector4vl.nor() called with vectors of different sizes");
        const [a1, b1] = this._z2x();
        const [a2, b2] = v._z2x();
        return new Vector4vl(this._bits,
            zip((a, b) => ~(a | b), b2, b1),
     ...
     */
    xnor(v : Vector4vl) {
        assert(v._bits == this._bits, "Vector4vl.xnor() called with vectors of different sizes");
        const [a1, b1] = this._z2x();
        const [a2, b2] = v._z2x();
        return new Vector4vl(this._bits,
            zip4((a1_, a2_, b1_, b2_) => ~((a1_ & b1_) ^ (a2_ | b2_)), a2, b2, a1, b1),
    ...
    not() {
        const [a, b] = this._z2x();
        return new Vector4vl(this._bits, b.map(x => ~x), a.map(x => ~x));
    }

    /**
     * Return a vector with 1 on locations with x or z, the rest with 0.
     */
    xmask() {
        const v = zip((a, b) => a ^ b, this._avec, this._bvec);
        return new Vector4vl(this._bits, v, v);
    }
    ...
    reduceAnd() {
        const [a, b] = this._z2x();
        return new Vector4vl(1,
            Uint32Array.of(bitfold((x, y) => x & y, a, this._lastmask, 1)),
            Uint32Array.of(bitfold((x, y) => x & y, b, this._lastmask, 1)));
    }
    ...
    reduceOr() {
        const [a, b] = this._z2x();
        return new Vector4vl(1,
            Uint32Array.of(bitfold((x, y) => x | y, a, this._lastmask, 0)),
            Uint32Array.of(bitfold((x, y) => x | y, b, this._lastmask, 0)));
    }
    ...
    reduceXor() {
        const [a, b] = this._z2x();
        const xes = zip((x, y) => ~x & y, a, b);
        const has_x = bitfold((x, y) => x | y, xes, this._lastmask, 0);
        const v = bitfold((x, y) => x ^ y, a, this._lastmask, 0);
        return new Vector4vl(1, Uint32Array.of(v & ~has_x), Uint32Array.of(v | has_x));
    }
     ...
     */
    concat(...vs : Vector4vl[]) {
        return Vector4vl.concat(this, ...vs);
    }
    ...
    slice(start? : number, end? : number) {
        ...
        if (bitnum(start) == 0) {
            ...
            return new Vector4vl(end - start, avec, bvec);
        } else {
    ...
    *toIteratorPow2(skip : number) {
     ...
     *
     * The resulting array contains values -1, 0, 1, 2.
     */
    toArray() {
        // copy-paste'y code for performance
        this.normalize();
     ...
     *
     * Returned characters can be '0' to '7', 'x' and 'z'. An 'x' value is
     * returned if the three bits are a mix of defined/undefined/
     * high-impedance values; a 'z' value is returned only if all three bits
     * are high-impedance.
     */
    toOct() {
        // copy-paste'y code for performance
        ...
        while (bit < this._bits) {
            ...
            const undef = 0x7 & (a ^ b);
            const zbits = 0x7 & a & ~b;
            if (!undef) out.push((v >> 3).toString());
            else if (zbits == 0x7) out.push('z');
            else out.push('x');
            m <<= skip;
            bit += skip;
            if (m == 0) {
     ...
     *
     * Returned characters can be '0' to '9', 'a' to 'f', 'x' and 'z'. An
     * 'x' value is returned if the four bits are a mix of defined/
     * undefined/high-impedance values; a 'z' value is returned only if all
     * four bits are high-impedance.
     */
    toHex() {
        this.normalize();
    ...
    toNumber(signed = false) {
        ...
        assert(this.isFullyDefined, "Vector4vl.toNumber() called on a not fully defined vector");
        assert(this._bits < 32, "Vector4vl.toNumber() called on a too wide vector");
    ...
    toNumberSigned() {
        assert(this.isFullyDefined, "Vector4vl.toNumberSigned() called on a not fully defined vector");
        assert(this._bits < 32, "Vector4vl.toNumberSigned() called on a too wide vector");
        assert(this._bits > 0, "Vector4vl.toNumberSigned() called on an empty vector");
    ...
    toBigInt(signed = false) {
        ...
        assert(this.isFullyDefined, "Vector4vl.toBigInt() called on a not fully defined vector");
    ...
    toBigIntSigned() {
        assert(this.isFullyDefined, "Vector4vl.toBigIntSigned() called on a not fully defined vector");
        assert(this._bits > 0, "Vector4vl.toBigIntSigned() called on an empty vector");
        const sign = this.msb == 1;
...
export class Mem4vl {
    ...
    constructor(bits : number, size : number, val? : number) {
        ...
        this._avec = new Uint32Array(size * this._wpc).fill(val === 2 || val > 0 ? ~0 : 0);
        this._bvec = new Uint32Array(size * this._wpc).fill(val === 2 ? 0 : (val >= 0 ? ~0 : 0));
        if (this._size) this.set(this._size - 1, this.get(this._size - 1)); // TODO faster
    }
    static fromData(data : Vector4vl[]) {
        if (data.length == 0) return new Mem4vl(0, 0);
        const ret = new Mem4vl(data[0].bits, data.length);
        for (const i in data) {
            data[i].normalize();
            assert(data[i].bits == ret._bits, "Mem4vl.fromData() called with vectors of different sizes");
    ...
    get(i : number) : Vector4vl {
        const idx = this._wpc * i;
        return new (Vector4vl as any)(this._bits,
            this._avec.slice(idx, idx+this._wpc),
            this._bvec.slice(idx, idx+this._wpc));
    }
    set(i : number, v : Vector4vl) {
        assert(v.bits == this._bits, "Mem4vl.set() called with a vector with different bit width than the memory");
        v.normalize();
    ...
    static fromJSON(bits, rep) {
        ...
        const ret = new Mem4vl(bits, size, -1);
        let w = 0;
...
export interface Display4vlInterface {
    readonly name : string;
    readonly sort : number;
    readonly pattern : string;
    can(kind : 'read' | 'show', bits : number) : boolean;
    read(data : string, bits? : number) : Vector4vl;
    show(data : Vector4vl) : string;
    validate(data : string, bits? : number) : boolean;
    size(bits : number) : number;
};

export class Display4vlWithRegex {
...
export class Display4vlHex extends Display4vlWithRegex implements Display4vlInterface {
    constructor() {
        super("[0-9a-fxz]*");
    }
    name : string = "hex";
    ...
    read(data : string, bits? : number) : Vector4vl {
        return Vector4vl.fromHex(data, bits);
    }
    show(data : Vector4vl) : string {
        return data.toHex();
    }
...
export class Display4vlBin extends Display4vlWithRegex implements Display4vlInterface {
    constructor() {
        super("[01xz]*");
    }
    name : string = "bin";
    ...
    read(data : string, bits? : number) : Vector4vl {
        return Vector4vl.fromBin(data, bits);
    }
    show(data : Vector4vl) : string {
        return data.toBin();
    }
...
export class Display4vlOct extends Display4vlWithRegex implements Display4vlInterface {
    constructor() {
        super("[0-7xz]*");
    }
    name : string = "oct";
    ...
    read(data : string, bits? : number) : Vector4vl {
        return Vector4vl.fromOct(data, bits);
    }
    show(data : Vector4vl) : string {
        return data.toOct();
    }
...
class Display4vlDec extends Display4vlWithRegex {
    constructor() {
        super('[0-9]*|x|z');
    }
    get name() {
    ...
    read(data, bits) {
        if (data == 'x') return Vector4vl.xes(bits);
        if (data == 'z') return Vector4vl.zes(bits);
...
class Display4vlDec2c extends Display4vlWithRegex {
    constructor() {
        super('-?[0-9]*|x|z');
    }
    get name() {
...
export class Display4vl {
    private readonly displays : Record<string, Display4vlInterface> = {};
    constructor() {
        this.addDisplay(new Display4vlHex());
        this.addDisplay(new Display4vlBin());
        this.addDisplay(new Display4vlOct());
        this.addDisplay(new Display4vlDec());
        this.addDisplay(new Display4vlDec2c());
    }
    addDisplay(display : Display4vlInterface) {
        this.displays[display.name] = display;
    }
    usableDisplays(kind : 'read' | 'show', bits : number) : string[] {
        const ret : Display4vlInterface[] = [];
    ...
    show(name : string, data : Vector4vl) : string {
        return this.displays[name].show(data);
    }
    read(name : string, data : string, bits? : number) : Vector4vl {
